...
var MoonCart = function(){
	...
	function addParamsAndReload(params) {
		// Создание объекта URL из текущего адреса
		var currentUrl = new URL(window.location.href);
		
		// Получение объекта URLSearchParams для управления параметрами запроса
		var searchParams = new URLSearchParams(currentUrl.search);
		
		// Добавление или обновление параметров
		Object.keys(params).forEach(key => {
			searchParams.set(key, params[key]); // set устанавливает или заменяет параметр
		});
		
		// Установка новой строки поиска для URL
		currentUrl.search = searchParams.toString();
		
		$("#acceptRangePrice").attr("href", currentUrl.href)
	}
	...
	var priceslider = function(){
		if($("#slider-tooltips").length > 0 ) {
			...
			var min = parseInt($("input[name=min_price]").val());
			var max = parseInt($("input[name=max_price]").val());
			noUiSlider.create(tooltipSlider, {
				...
				range: {
					'min': parseInt($("input[name=total_min_price]").val()),
					'max': parseInt($("input[name=total_max_price]").val())
				}
			});
			var formatValues = [
			...
			tooltipSlider.noUiSlider.on('update', function (values, handle, unencoded) {
				formatValues[0].innerHTML = "Мін. ціна: " + "UAH"+ values[0];
				formatValues[1].innerHTML = "Макс. ціна: " + "UAH"+ values[1];
			});
